...
function mapApiTaskToUi(task: ApiTask): MockTask {
  return {
    ...
    assignees: (task.assignees ?? []).map((u) => ({ id: u.id, name: u.username, initials: u.username?.[0]?.toUpperCase() ?? '?' })),
    labels: task.labels ?? [],
    resources: task.resources ?? [],
    subtasks: task.subtasks ?? [],
    reporter: task.owner ? { id: task.owner.id, name: task.owner.username, initials: task.owner.username?.[0]?.toUpperCase() ?? '?' } : null,
  };
}
...
function mapApiProjectToUi(p: ApiProject): UiProject {
  return { id: p.id, name: p.name, lead: p.lead ? { id: p.lead.id, username: p.lead.username } : null, members: (p.members ?? []).map((m) => ({ id: m.id, username: m.username })) };
}
...
export async function fetchUsers(): Promise<UiAppUser[]> {
  return (await apiFetch<ApiUser[]>('/users')).map((u) => ({ id: u.id, username: u.username }));
}
...
export async function fetchComments(taskId: string): Promise<UiComment[]> {
  return (await apiFetch<ApiComment[]>(`/tasks/${taskId}/comments`)).map((c) => ({ id: c.id, author: c.author?.username ?? 'Unknown', body: c.body, createdAt: c.createdAt }));
}
